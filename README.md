...
- [nika](https://github.com/supernovae-st/nika) - (AGPL 开源/本地运行) Rust 编写的 AI 工作流引擎：.nika.yaml 声明式 DAG，执行前静态检查（schema/权限/成本预估），执行后生成防篡改 trace；本地优先（Ollama/llama.cpp/vLLM，离线 mock），内置 MCP 客户端 + 服务端。
